...
class CSEService:
    ...
    def get_stock_prices(self, symbol: str, period: str = "5") -> pd.DataFrame:
        """
        Retrieves stock prices. Tries API first, falls back to CSV, then to synthetic generation.
        """
        ...
        try:
            ...
            df = self.parser.parse_csv_data(formatted_symbol, csv_data)
            if not df.empty:
                return df
        except Exception as csv_err:
            logger.error(f"Fallback CSV loading failed for {symbol}: {csv_err}")

        # Final fallback: Synthetic GBM data generation
        try:
            from app.data_sources.cse.yfinance_client import YFinanceCSEClient
            yf = YFinanceCSEClient()
            return yf._generate_synthetic(symbol)
        except Exception as syn_err:
            logger.error(f"Synthetic fallback failed for {symbol}: {syn_err}")
